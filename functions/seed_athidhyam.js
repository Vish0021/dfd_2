...
async function seedData() {
  try {
    ...
    const storeRef = db.collection('stores').doc('athidhyam-store');
    ...
    for (const [categoryName, items] of Object.entries(rawMenu)) {
      // Create category doc inside store subcollection if using normalized data
      // For simplicity matching the schema, let's just create global items linking back to `storeId` and `categoryId`.
      
      const niceCategoryName = categoryName.replace(/_/g, " ");

      const catRef = db.collection('categories').doc(`athidhyam-${categoryName.toLowerCase()}`);
      batch.set(catRef, {
      ...
      for (const item of items) {
        ...
        batch.set(itemRef, {
          ...
          itemType: categoryName === 'DESERTS' || categoryName === 'ICE_CREAM' ? 'veg' : item.name.toLowerCase().includes('chicken') || item.name.toLowerCase().includes('mutton') || item.name.toLowerCase().includes('fish') || item.name.toLowerCase().includes('prawns') || item.name.toLowerCase().includes('egg') ? 'non-veg' : 'veg',
          createdAt: admin.firestore.FieldValue.serverTimestamp()
        });
        itemsCreated++;
      }
