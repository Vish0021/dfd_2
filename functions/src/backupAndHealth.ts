...
const COLLECTIONS_TO_EXPORT: string[] = []; // all collections
 ...
 */
async function cleanupOldBackups(fn: string): Promise<void> {
  const now = new Date();
...
export const healthCheck = onRequest(
  ...
  async (_req, res) => {
    ...
    try {
      ...
      logger.info("Health check passed", { functionName: FN, ...payload });
